...
var MinOglasaKlase=100;
var NoviOglasi=[];
var Uprocesu=0;
var DEBUG_MODE="FULL_DEBUG";
            ...
            function wrapp(PageNum) 
            {
                ...
                SpecialCase.addEveryTime(Route,PageNum,UzmiSve,function(specialArr)
                {
                        ...
                        var options =
                        {
                            ...
                            GetData.GetRawData('https://'+options.host+options.path,Route.phantomSupport,Route.websitename,UzmiSve,function(err,resp,body)
                            {
                                if(body==-1)
                                {
                                    ...
                                    for(klasa in Route.class) 
                                    {
                                        ...
                                        sm(Route.class[klasa]).each(function(i,j) 
                                        {
                                           ...
                                           /* var velikitemp=obj.cena;
                                            ...
                                            if(obj.link[0]=='/')obj.link=Route.linkPrefix+Route.host+obj.link;
                            ...
                            if(!n)
                            {
                                ...
                                if(i.shouldCrawl)
                                {
                                    ...
                                    crawl.find(i,UzmiSve,function(resp)
                                    {
                                        ...
                                        if(resp==-1)
                                        {
                                            console.log("SHEEEEEEEEEEEEEEEEEEATTTTTTTT")
                                            websiteWontRespond=1;
                                            finish()
                                        }
                                        else
                                        {
                                            resp.cena=unit.getPriceInDefaultUnit(resp.cena);
                                           // console.log("fixed price:"+resp.cena);
                                           // console.log(resp.link)
                                            resp.kvadratura=unit.getKvadraturaInDefaultUnit(resp.kvadratura);
                                            changeDataType(resp);
